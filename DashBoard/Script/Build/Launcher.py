...
def main():
    ...
    # 根目录与Launcher构建目录（适配Launcher项目路径）
    root_dir = os.getcwd()
    build_dir = os.path.join(root_dir, "Build", "Launcher")  # 改为Launcher的构建目录
    print(f"Build directory: {build_dir}")

    # 原始Release目录路径（Launcher的编译输出）
    original_release_dir = os.path.join(build_dir, "Release")
    exe_name = "Launcher.exe" if current_os == "Windows" else "Launcher"  # 改为Launcher的可执行文件名
    exe_path = os.path.join(original_release_dir, exe_name)

    # QML文件所在目录（Launcher的Qml目录：根目录/Src/Launcher/Qml/）
    qml_dir = os.path.join(root_dir, "Src", "Launcher", "Qml")  # 改为Launcher的QML路径
    ...
    # 1. 彻底清理build/Launcher下除Release外的所有文件和文件夹
    ...
    # 3. Windows下使用正确的windeployqt命令打包（含Launcher的QML目录）
    if current_os == "Windows":
        print("Packaging with windeployqt (including QML files)...")
        windeployqt_path = r"D:\Qt\6.9.3\msvc2022_64\bin\windeployqt.exe"  # 与Insight共用Qt路径
        ...
        try:
            # 适配Launcher：用Launcher的exe和QML目录执行打包
            subprocess.run(
